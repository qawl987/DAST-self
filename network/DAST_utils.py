...
class Sensor_MultiHeadAttentionBlock(torch.nn.Module):
    ...
    def forward(self, x, kv=None):
        a = []
        if DEBUG == True:
            print('Sensor MHA input: ', x.shape)
        for h in self.heads:
            a.append(h(x, kv=kv))
        a = torch.stack(a, dim=-1)
        a = a.flatten(start_dim=2)
        x = self.fc(a)
        if DEBUG == True:
            print('Sensor MHA output: ', x.shape)
        return x
...
class Query(torch.nn.Module):
    def __init__(self, dim_input, dim_attn):
        ...
        self.dim_attn = dim_attn
        self.fc1 = nn.Linear(dim_input, dim_attn, bias=False)
        # self.register_buffer('fc1', nn.Linear(dim_input, dim_attn, bias = False))

    def forward(self, x):
        # print(next(self.fc1.parameters()).device)
        x = self.fc1(x)
        return x
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=512):
        ...
        div_term_even = torch.exp(torch.arange(
            0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        div_term_odd = torch.exp(torch.arange(
            1, d_model, 2).float() * (-math.log(10000.0) / d_model))
